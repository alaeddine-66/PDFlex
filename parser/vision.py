...
class BaseVisionExtractor(BaseExtractor):
    ...
    def extract(self, file_path: Path) -> tuple[str, dict[str, Any]]:
        ...
        if self._is_image(file_path):
            ...
            for page in doc:
                ...
                text = self.process_single_image_from_bytes(b64)
                pages_text.append(text)
            doc.close()
    ...
    @abstractmethod
    def process_single_image_from_bytes(self, b64_string: str) -> str:
        """To be implemented by Tesseract or Vision subclasses."""
        ...
...
class LLMVisionExtractor(BaseVisionExtractor):
    ...
    def _call_llm(self, b64_image: str) -> str:
        llm = get_llm()
        prompt = (
        "You are a strict OCR engine. Your ONLY task is to extract text from the provided image. "
            "Transcribe it exactly as it appears, even if the letters are separated, scattered, or faint. "
            "DO NOT output any conversational text, explanations, or Markdown formatting. "
            "If there is absolutely no readable text in the image, output EXACTLY the word 'NONE' and nothing else."
        )
        response = llm.invoke([
    ...
    def process_single_image_from_bytes(self, b64: str) -> str:
        return self._call_llm(b64)
